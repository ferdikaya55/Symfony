...
class ImageController extends AbstractController
{
    ...
    public function new(Request $request, $pid, ImageRepository $imageRepository): Response
    {
        $imagelist = $imageRepository->findBy(
            ['product_id' => $pid]); // product id si bizim gönderdiğimiz id olan dosyaların listesi

        $image = new Image();
        $form = $this->createForm(ImageType::class, $image);
        $form->handleRequest($request);
        if ($request->files->get('imagename')) //if ($form->isSubmitted() && $form->isValid())
        {
            // FİLE UPLOAD İŞLEMLERİ
            ...
            return $this->redirectToRoute('admin_image_new',
                array('pid' => $pid)); // resmi gönderiyoruz upload edip tekrar galeriye gelmemiz gerek
        }
    ...
    /**
     * @Route("/{id}/{pid}", name="admin_image_sil", methods="GET")
     */
    // Silinecek ürün id si ile resmin id si farklı şeyler. id yi geri dönmek için kullandık
    public function sil(Request $request, Image $image,$pid): Response
    {
